...
This connector imports MISP feed data into OpenCTI.
Includes specific fixes for feeds like Rosti that regenerate their manifest daily.
"""
...
class MispFeed:
    ...
    def _process_attribute(
        ...
        if attribute.get("type") == "link" and attribute.get("category") == "External analysis":
            return None
        resolved_attributes = self._resolve_type(attribute.get("type", ""), attribute.get("value", ""))
        ...
        for resolved_attribute in resolved_attributes:
            ...
            name = (
                ...
                if len(resolved_attribute["value"]) > 2
                    ...
                    if len(attribute.get("comment", "")) > 2
            ...
            if self.config.misp_feed.create_indicators and pattern is not None:
                try:
                    timestamp_value = int(attribute.get("timestamp", time.time()))
                    attribute_datetime = datetime.fromtimestamp(
    ...
    def _process_event(self, event) -> str:
        ...
        if author is None:
            orgc = event["Event"].get("Orgc", {})
            orgc_name = orgc.get("name", "Unknown")
            author = stix2.Identity(
                id=Identity.generate_id(orgc_name, "organization"),
                name=orgc_name,
                identity_class="organization",
            )
        if "Tag" in event["Event"]:
        ...
        for object in event["Event"].get("Object", []):
            attribute_external_references = []
            for attribute in object.get("Attribute", []):
                if (
                    attribute.get("type") == "link"
                    and attribute.get("category") == "External analysis"
                ):
            ...
            if self.config.misp_feed.create_object_observables:
                if (
                    ...
                    if len(object.get("Attribute", [])) > 0:
                        value = object["Attribute"][0].get("value", "")
                    ...
                    if len(object.get("Attribute", [])) > 0:
                        unique_key = (
                            " ("
                            + object["Attribute"][0].get("type", "")
                            + "="
                            + object["Attribute"][0].get("value", "")
                            + ")"
                        )
                    object_observable = CustomObservableText(
                        value=object.get("name", "unknown") + unique_key,
                        object_marking_refs=event_markings,
                        custom_properties={
                            "description": object.get("description", ""),
                            "x_opencti_score": self._threat_level_to_score(
                    ...
                    objects_observables.append(object_observable)
            object_attributes = []
            create_relationships = len(object.get("Attribute", [])) < 10000
            for attribute in object.get("Attribute", []):
                indicator = self._process_attribute(
                    author,
                    event_elements,
                ...
                if indicator is not None:
                    ...
                    if (
                        ...
                        and object.get("meta-category") == "file"
                        and (
        ...
        if self.config.misp_feed.create_reports:
            ...
            report = stix2.Report(
                ...
                modified=datetime.fromtimestamp(
                    int(event["Event"].get("timestamp", time.time())), tz=timezone.utc
                ).strftime("%Y-%m-%dT%H:%M:%SZ"),
            ...
            for note in event["Event"].get("EventReport", []):
                try:
                    note_ts = int(note.get("timestamp", time.time()))
                    note_content = note.get("content", "")
                    note = stix2.Note(
                        id=Note.generate_id(
                            datetime.fromtimestamp(
                                note_ts, tz=timezone.utc
                            ).strftime("%Y-%m-%dT%H:%M:%SZ"),
                            self._process_note(note_content, bundle_objects),
                        ),
                        created=datetime.fromtimestamp(
                            note_ts, tz=timezone.utc
                        ).strftime("%Y-%m-%dT%H:%M:%SZ"),
                        ...
                        content=self._process_note(note_content, bundle_objects),
                        object_refs=[report],
                        allow_custom=True,
                    )
                    bundle_objects.append(note)
                except Exception as e:
                    self.helper.log_error(f"Error processing EventReport note: {e}")
        ...
        try:
            ...
            if self.config.misp_feed.source_type == "url":
                ...
                if (
                    ...
                    and "last_event_date" in current_state
                ):
                    last_run = parse(current_state["last_run"])
                    last_event_date = current_state["last_event_date"]
                    processed_uuids = set(
                    ...
                    self.helper.log_info(
                        "Connector last event date: "
                        + last_event_date
                        + " ("
                        + str(len(processed_uuids))
                        + " UUIDs already processed)"
                    )
                else:
                    if self.config.misp_feed.import_from_date is not None:
                        last_event_date = self.config.misp_feed.import_from_date.strftime(
                            "%Y-%m-%d"
                        )
                    else:
                        last_event_date = now.strftime("%Y-%m-%d")
                    processed_uuids = set()
                    self.helper.log_info("Connector has never run")
                self.helper.log_info(
                    "Filtering events with Event.date >= " + last_event_date
                )
                number_events = 0
                # Track the max date seen for updating state at the end
                # Don't update last_event_date during processing to avoid
                # skipping events (manifest UUIDs are not ordered by date)
                max_event_date = last_event_date
                try:
                    # Manifest is only used as a list of UUIDs.
                    # Manifest date/timestamp fields are unreliable
                    # (regenerated daily by some feeds like Rosti).
                    manifest_data = json.loads(
                        self._retrieve_data(
                            self.config.misp_feed.url + "/manifest.json"
                        )
                    )
                    all_uuids = list(manifest_data.keys())
                    total_items = len(all_uuids)
                    self.helper.log_info(
                        "Manifest contains "
                        + str(total_items)
                        + " events, "
                    ...
                    failed_events = 0
                    filtered_events = 0
                    for idx, item_uuid in enumerate(all_uuids, 1):
                        # Skip already-processed UUIDs (fast, no download)
                        if item_uuid in processed_uuids:
                            skipped_events += 1
                            continue
                        # Download the actual event JSON
                        try:
                            event_data = self._retrieve_data(
                            ...
                            failed_events += 1
                            continue
                        # Get the REAL date from the event JSON
                        evt = event.get("Event", event)
                        event_date = evt.get("date", "")
                        event_info = evt.get("info", "?")
                        # Filter on real Event.date
                        if event_date < last_event_date:
                            # Don't add to processed_uuids - we want to reprocess
                            # if last_event_date changes (e.g. after state reset)
                            filtered_events += 1
                            continue
                        self.helper.log_info(
                            "Processing event "
                        ...
                        try:
                            # Propagate event timestamp to attributes missing their own
                            event_ts = evt.get("timestamp")
                            if event_ts:
                                for attr in evt.get("Attribute", []):
                                    if not attr.get("timestamp"):
                                        attr["timestamp"] = event_ts
                                for obj in evt.get("Object", []):
                        ...
                        self.helper.log_info(
                            "Event processed (max_event_date="
                            + max_event_date
                            + ", processed="
                            + str(number_events)
                            + ")"
                ...
                message = (
                    "Connector successfully run ("
                    + str(number_events)
                    + " events have been processed), last_event_date="
                    + max_event_date
                    + ", "
                    + str(len(processed_uuids))
                    + " UUIDs tracked)"
